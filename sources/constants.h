...
// the box. Typical C++ L.
inline const u64 gJfaSteps{static_cast<u64>(
    std::ceil(std::log2(std::max(gScreenHeight, gScreenWidth))))};

inline constexpr u64 gMaxBrushRadius{50};

inline const f32 gBrushScale{1.0f /
                             std::sqrtf((gScreenWidth * gScreenWidth) +
...
inline constexpr u64 gFramesToMeasure{512};
